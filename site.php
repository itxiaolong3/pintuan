...
require IA_ROOT.'/addons/pintuan/inc/func/core.php';
class pintuanModuleSite extends Core {
    ...
    public function doMobileNewOrder(){
        ...
        $res2=pdo_fetch($sql);
       // $res2=pdo_get('pintuan_order',array('time2 >='=>,'type'=>2,'store_id'=>$store_id));//店内
        if($res){
            echo 1;
        }elseif($res2){
            echo 2;
    ...
    public function doMobileJdOrder(){
        global $_W,$_GPC;
          $store_id=$_GPC['id'];
             $store=pdo_get('pintuan_store',array('id'=>$store_id));
             if($store['is_jd']==1){
                $time=time()-$store['jd_time'];
                $data['state']=3;
                $res=pdo_update("pintuan_order",$data,array('store_id'=>$store_id,'time2 <='=>$time,'state'=>2));
                if($res){
                    echo  '1';
                }else{
                    echo  '2';
    ...
    public function doMobileUpdUser(){
        ...
        $res=pdo_delete('pintuan_user',array('id'=>$_GPC['id']));
         if($res){
            echo '1';
        }else{
            echo '2';
  ...
  //批量删除
    public function doMobileUpdCai(){
        ...
        foreach ($getidarr as $k=>$v){
            $res=pdo_update('pintuan_goodmy',array('isdelete'=>1),array('gID'=>$v));
        }
        if($res){
            echo  '1';
        }else{
    ...
    public function doMobileDelCz(){
		global $_W,$_GPC;
		$res=pdo_delete('pintuan_czhd',array('id'=>$_GPC['id']));
		if($res){
			echo '1';
		}else{
			echo '2';
...
//下架
    public function doMobileXj(){
      ...
      $getidarr=$_GPC['id'];
        foreach ($getidarr as $k=>$v){
            $res=pdo_update('pintuan_goodmy',array('Statu'=>2),array('gID'=>$v));
        }
        if($res){
            echo  '1';
        }else{
  ...
  //上架
     public function doMobileSj(){
    	...
    	$getidarr=$_GPC['id'];
         foreach ($getidarr as $k=>$v){
             $res=pdo_update('pintuan_goodmy',array('Statu'=>1),array('gID'=>$v));
         }
         if($res){
             echo  '1';
         }else{
    ...
    public function doMobileTime(){
        ...
        for($i=1;$i<=$day;$i++){
            ...
            $time=$y."-".$m."-".$i;
            $time="'%$time%'";
            $wm = "select sum(money) as total from " . tablename("pintuan_order")." WHERE time LIKE ".$time." and store_id=".$storeid." and state not in (5,1,8) and type=1 and pay_time !=''";
            $wm = pdo_fetch($wm);//外卖销售额
            $dn = "select sum(money) as total from " . tablename("pintuan_order")." WHERE time LIKE ".$time." and store_id=".$storeid." and dn_state not in (3,1) and type=2 and pay_time !=''";
            $dn = pdo_fetch($dn);//店内销售额
            $yd = "select sum(pay_money) as total from " . tablename("pintuan_ydorder")." WHERE created_time LIKE ".$time." and store_id=".$storeid." and state not in (0,6)";
            $yd = pdo_fetch($yd);//预定销售额
            $dmf = "select sum(money) as total from " . tablename("pintuan_dmorder")." WHERE time LIKE ".$time." and state=2 and store_id=".$storeid;
            $dmf = pdo_fetch($dmf);//当面付销售额
            $total = $wm['total']+$dn['total']+$yd['total']+$dmf['total'];//销售额


            $wm2 = "select * from " . tablename("pintuan_order")." WHERE time LIKE ".$time." and store_id=".$storeid." and state not in (5,1,8) and type=1 and pay_time !=''";
            $wm2 = count(pdo_fetchall($wm2));//外卖销售量

            $dn2 = "select * from " . tablename("pintuan_order")." WHERE time LIKE ".$time." and store_id=".$storeid." and dn_state not in (3,1) and type=2 and pay_time !=''";
            $dn2 = count(pdo_fetchall($dn2));//店内销售量
            $yd2 = "select * from " . tablename("pintuan_ydorder")." WHERE created_time LIKE ".$time." and store_id=".$storeid." and state not in (0,6)";
            $yd2 = count(pdo_fetchall($yd2));//预定销售量
            $dmf2 = "select * from " . tablename("pintuan_dmorder")." WHERE time LIKE ".$time." and state=2 and store_id=".$storeid;
...
public function doMobileSelectUser(){
  ...
  // echo  $_GPC['keywords'];
    //查出已是商家用户
$sjuser=pdo_getall('pintuan_store',array('uniacid'=>$_W['uniacid']),'user_id');
//二维数组转一维
...
if($yuser){
$sql =" select id,name from ".tablename('pintuan_user')." where uniacid={$_W['uniacid']}  and id not in ({$string}) and  (name like '%{$_GPC['keywords']}%' || openid like '%{$_GPC['keywords']}%') and name !=''";  
}else{
 $sql =" select id,name from ".tablename('pintuan_user')." where uniacid={$_W['uniacid']}   and  (name like '%{$_GPC['keywords']}%' || openid like '%{$_GPC['keywords']}%') and name !=''";     
}
...
public function doMobileSelectUser2(){
  ...
  // echo  $_GPC['keywords'];
    //查出已是商家用户
$sjuser=pdo_getall('pintuan_store',array('uniacid'=>$_W['uniacid']),'admin_id');
//二维数组转一维
function i_array_column($input, $columnKey, $indexKey=null){
    if(!function_exists('array_column')){ 
        $columnKeyIsNumber  = (is_numeric($columnKey))?true:false; 
...
public function doMobileJfGoodsSj(){
     global $_W, $_GPC;
        $res=pdo_update('pintuan_jfgoods',array('is_open'=>1),array('id'=>$_GPC['id']));
        if($res){
...
public function doMobileJfGoodsXj(){
     global $_W, $_GPC;
        $res=pdo_update('pintuan_jfgoods',array('is_open'=>2),array('id'=>$_GPC['id']));
        if($res){
            message('操作成功',$this->createWebUrl('jfgoods',array()),'success');
        }else{
            message('操作失败','','error');
...
public function doMobileDelJfGoods(){
     global $_W, $_GPC;
        $res=pdo_delete('pintuan_jfgoods',array('id'=>$_GPC['id']));
        if($res){
...
public function doMobileqgGoodsSj(){
     global $_W, $_GPC;
        $res=pdo_update('pintuan_qggoods',array('state'=>1),array('id'=>$_GPC['id']));
        if($res){
...
public function doMobileqgGoodsXj(){
     global $_W, $_GPC;
        $res=pdo_update('pintuan_qggoods',array('state'=>2),array('id'=>$_GPC['id']));
        if($res){
            message('操作成功',$this->createWebUrl('qggoods',array()),'success');
        }else{
            message('操作失败','','error');
...
public function doMobileqgGoodsXs(){
     global $_W, $_GPC;
        $res=pdo_update('pintuan_qggoods',array('state2'=>1),array('id'=>$_GPC['id']));
        if($res){
            message('操作成功',$this->createWebUrl('qggoods',array()),'success');
        }else{
            message('操作失败','','error');
...
public function doMobileqgGoodsYc(){
     global $_W, $_GPC;
        $res=pdo_update('pintuan_qggoods',array('state2'=>2),array('id'=>$_GPC['id']));
        if($res){
            message('操作成功',$this->createWebUrl('qggoods',array()),'success');
        }else{
            message('操作失败','','error');
...
public function doMobileDelqgGoods(){
 ...
 //添加商品 已废除
    public function doMobileAddGood11111(){
        global $_W,$_GPC;
        $data['tid']=$_GPC['tid'];
    ...
    public function doMobileUpdGood(){
      ...
      // print_r($_GPC['menu']);die;
        if($_GPC['scid']){
              pdo_delete('pintuan_spec_combination',array('good_id'=>$_GPC['good_id']));
            pdo_delete('pintuan_shopcar',array('good_id'=>$_GPC['good_id']));
          for($t=0;$t<count($_GPC['scid']);$t++){
            pdo_delete('pintuan_spec',array('id'=>$_GPC['scid'][$t]['id']));
            pdo_delete('pintuan_spec_val',array('spec_id'=>$_GPC['scid'][$t]['id']));
        ...
        if($_GPC['smallid']){
            ...
            pdo_delete('pintuan_spec_val',array('id'=>$_GPC['smallid'][$y]['id']));
            
        }  
        }
        $data['name']=$_GPC['goodname'];
        $data['type_id']=$_GPC['fenlei'];
        $goodlist=pdo_get('pintuan_goods',array('id'=>$_GPC['good_id']));
        ...
        $res=pdo_update('pintuan_goods',$data,array('id'=>$_GPC['good_id']));   
        for($i=0;$i<count($_GPC['list']);$i++){
            if($_GPC['list'][$i]['coid']){
                pdo_update('pintuan_spec',array('name'=>$_GPC['list'][$i]['color']),array('id'=>$_GPC['list'][$i]['coid']));
            }else{
                pdo_delete('pintuan_spec_combination',array('good_id'=>$_GPC['good_id']));
                pdo_delete('pintuan_shopcar',array('good_id'=>$_GPC['good_id']));
                pdo_insert('pintuan_spec',array('name'=>$_GPC['list'][$i]['color'],'good_id'=>$_GPC['good_id'],'num'=>$i,'uniacid'=>$_W['uniacid']));
                $specid=pdo_insertid();
            }
            
            for($j=0;$j<count($_GPC['list'][$i]['ggarr']);$j++){
                if(is_numeric($_GPC['list'][$i]['ggarr'][$j]['shuid'])){
                    pdo_update('pintuan_spec_val',array('name'=>$_GPC['list'][$i]['ggarr'][$j]['guigebig']),array('id'=>$_GPC['list'][$i]['ggarr'][$j]['shuid']));
                }else{
                    if($_GPC['list'][$i]['coid']){
                    ...
                    pdo_delete('pintuan_spec_combination',array('good_id'=>$_GPC['good_id']));
                    pdo_delete('pintuan_shopcar',array('good_id'=>$_GPC['good_id']));
                    pdo_insert('pintuan_spec_val',array('name'=>$_GPC['list'][$i]['ggarr'][$j]['guigebig'],'spec_id'=>$spec_id,'num'=>$j,'uniacid'=>$_W['uniacid'],'good_id'=>$_GPC['good_id']));
                }
            }
        }
        for($k=0;$k<count($_GPC['menu']);$k++){
            ...
            if($_GPC['menu'][$k]['id']){
                pdo_update("pintuan_spec_combination",$data4,array('id'=>$_GPC['menu'][$k]['id']));
            }else{
                pdo_insert("pintuan_spec_combination",$data4);
            }
...
//查看商品详情
    public function doMobileGoodInfo(){
        global $_W,$_GPC;
        $type=pdo_getall('pintuan_spec',array('uniacid'=>$_W['uniacid'],'good_id'=>$_GPC['good_id']),array(),'','num ASC');
         $list=pdo_getall('pintuan_spec_val',array('uniacid'=>$_W['uniacid'],'good_id'=>$_GPC['good_id']),array(),'','num ASC');
         $data2=array();
        ...
        $combination=pdo_getall('pintuan_spec_combination',array('good_id'=>$_GPC['good_id']));
        $res['spec']=$data2;
...
public function doMobileNewCall(){
 ...
 if($_GPC['type']==1){
    $res=pdo_get('pintuan_call',array('store_id'=>$_GPC['store_id']));
    $src= $res['src'];
    echo  json_encode($src);
}
...
public function doMobileQueryNumber(){
   ...
   for($i=0;$i<$num;$i++){
    ...
    public function doMobileDelCall(){
        global $_W,$_GPC;
        $res=pdo_delete('pintuan_calllog',array('id'=>$_GPC['id']));
        if($res){
            echo '1';
        }else{
            echo '2';
...
//获取一级
   public function doMobileGetYj(){
        ...
        $sql=" select b.name as yj_name,c.name as fx_name,a.time from".tablename('pintuan_fxuser')." a left join ".tablename('pintuan_user')." b on a.fx_user=b.id left join ".tablename('pintuan_user')." c on a.user_id=c.id where a.user_id={$user_id}";
        $res=pdo_fetchall($sql);
        //$res=pdo_getall('pintuan_fxuser',array('user_id'=>$user_id));
        echo json_encode($res);     
}
//获取二级
   public function doMobileGetEj(){
        global $_W,$_GPC;
        $user_id=$_GPC['user_id'];
        $fx=pdo_get('pintuan_user',array('id'=> $user_id),'name');
        $res=pdo_getall('pintuan_fxuser',array('user_id'=>$user_id),'fx_user');
      
        foreach ($res as $key => $value) {
              $sql=" select b.name as yj_name,a.time from".tablename('pintuan_fxuser')." a left join ".tablename('pintuan_user')." b on a.fx_user=b.id  where a.user_id={$value['fx_user']}";
...
public function  doMobileAllAdopt(){
     global $_W,$_GPC;
     $res=pdo_update('pintuan_retail',array('state'=>2),array('id'=>$_GPC['id']));
     echo json_encode($res);
}
...
public function  doMobileAllDelTable(){
     global $_W,$_GPC;
     $res=pdo_delete('pintuan_table',array('id'=>$_GPC['id']));
     echo json_encode($res);
}
...
////下架
    public function doMobileGroupXj(){
        global $_W,$_GPC;
        $res=pdo_update('pintuan_groupgoods',array('is_shelves'=>2),array('id'=>$_GPC['id']));
        if($res){
            echo  '1';
        }else{
            echo  '2';
     ...
     public function doMobileGroupSj(){
        global $_W,$_GPC;
        $res=pdo_update('pintuan_groupgoods',array('is_shelves'=>1),array('id'=>$_GPC['id']));
        if($res){
            echo  '1';
        }else{
            echo  '2';
    ...
    public function doMobileDelGroup(){
        global $_W,$_GPC;
        $res=pdo_delete('pintuan_groupgoods',array('id'=>$_GPC['id']));
        if($res){
            echo  '1';
        }else{
            echo  '2';
    ...
    global $_W, $_GPC;
    $sql =" select id,name from ".tablename('pintuan_user')." where uniacid={$_W['uniacid']} and id not in (select hx_id  from" .tablename('pintuan_grouphx')."where store_id={$_GPC['store_id']} )  and name != '' and  openid like '%{$_GPC['keywords']}%'";    
    ...
    public function doMobileDelQd(){
        global $_W,$_GPC;
        $res=pdo_delete('pintuan_continuous',array('id'=>$_GPC['id']));
        if($res){
            echo '1';
        }else{
            echo '2';
    ...
    public function doMobileAddQd(){
        global $_W,$_GPC;
        pdo_delete('pintuan_continuous',array('uniacid'=>$_W['uniacid']));
        for($i=0;$i<count($_GPC['list']);$i++){
            $data['day']=$_GPC['list'][$i]['day'];
            $data['integral']=$_GPC['list'][$i]['integral'];
            $data['uniacid']=$_W['uniacid'];
            pdo_insert('pintuan_continuous',$data);
        }
        $data2['one']=$_GPC['one'];
        ...
        $res=pdo_get('pintuan_signset',array('uniacid'=>$_W['uniacid']));
        if($res){
            pdo_update('pintuan_signset',$data2,array('uniacid'=>$_W['uniacid']));
        }else{
            $data2['uniacid']=$_W['uniacid'];
            pdo_insert('pintuan_signset',$data2);
        }
        ...
        if($res2){
            ...
            pdo_update('pintuan_special',$data3,array('uniacid'=>$_W['uniacid']));
        }else{
            $data3['uniacid']=$_W['uniacid'];
            pdo_insert('pintuan_special',$data3);
        }
    }
    ...
    public function doMobileSelectStore(){
        global $_W, $_GPC;
        $sql =" select id,name from ".tablename('pintuan_store')." where uniacid={$_W['uniacid']} and state=2 and  name like '%{$_GPC['keywords']}%'";    
        echo json_encode(pdo_fetchall($sql));
    }
