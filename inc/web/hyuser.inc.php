...
if($_GPC['keywords']){
	...
	$pagesize=10;
	$sql="select *  from " . tablename("pintuan_user") ." WHERE  name LIKE :name  and uniacid=:uniacid and name!='' and UNIX_TIMESTAMP(dq_time)>=".time()." order by id DESC";
	$select_sql =$sql." LIMIT " .($pageindex - 1) * $pagesize.",".$pagesize;
	$list = pdo_fetchall($select_sql,array(':uniacid'=>$_W['uniacid'],':name'=>$where));	   
	$total=pdo_fetchcolumn("select count(*) from " . tablename("pintuan_user") ." WHERE  name LIKE :name  and uniacid=:uniacid  and name!='' and UNIX_TIMESTAMP(dq_time)>=".time(),array(':uniacid'=>$_W['uniacid'],':name'=>$where));
	$pager = pagination($total, $pageindex, $pagesize);
	if($_GPC['id']){
		$res4=pdo_delete("pintuan_user",array('id'=>$_GPC['id']));
		if($res4){
