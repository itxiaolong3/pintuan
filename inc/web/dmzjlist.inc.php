...
//获取商家手续费
$sql="select b.dm_poundage from".tablename('pintuan_store')."a  left join ".tablename('pintuan_storetype')." b on a.md_type=b.id where a.id={$storeid}";
	$list4=pdo_fetch($sql);
...
if(checksubmit('export_submit', true)) {
  $start=strtotime($_GPC['time']['start']);
  $end=strtotime($_GPC['time']['end']);
  $count = pdo_fetchcolumn("SELECT COUNT(*) FROM". tablename("pintuan_order")." WHERE type=4 and store_id={$storeid} and pay_type in (1,2) and dm_state=2 and UNIX_TIMESTAMP(time) >='{$start}' and UNIX_TIMESTAMP(time)<='{$end}'");
  ...
  for ($j = 1; $j <= $pagesize; $j++) {
    $sql = "select * from " . tablename("pintuan_order")." WHERE type=1 and store_id={$storeid} and pay_type in (1,2) and dm_state=2 and UNIX_TIMESTAMP(time) >='{$start}' and UNIX_TIMESTAMP(time)<='{$end}' ORDER BY id DESC limit " . ($j - 1) * 5000 . ",5000 ";
