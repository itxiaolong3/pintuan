...
if(checksubmit('export_submit', true)) {
  ...
  for ($j = 1; $j <= $pagesize; $j++) {
    $sql = "select a.*,b.name as md_name,c.name as table_name from " . tablename("pintuan_order")."  a"  . " inner join " . tablename("pintuan_store")." b on a.store_id=b.id inner join " . tablename("pintuan_table")." c on a.table_id=c.id WHERE a.type=2 and a.time >='{$start}' and a.store_id={$storeid} and a.time<='{$end}' limit " . ($j - 1) * 5000 . ",5000 ";
