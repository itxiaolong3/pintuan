...
//总数统计
$sql2="select sum(money) as 'total_money' from" . tablename("pintuan_qgorder") ." as a".$where;
...
$list4=pdo_fetch($sql);
$sql="SELECT a.* FROM ".tablename('pintuan_qgorder'). " a".$where." ORDER BY a.id DESC";
$total=pdo_fetchcolumn("SELECT count(*) FROM ".tablename('pintuan_qgorder'). " a"  .$where." ORDER BY a.id DESC",$data);
...
if(checksubmit('export_submit', true)) {
  $start=strtotime($_GPC['time']['start']);
  $end=strtotime($_GPC['time']['end']);
  $count = pdo_fetchcolumn("SELECT COUNT(*) FROM". tablename("pintuan_qgorder")." WHERE  store_id={$storeid}  and state in (2,3) and UNIX_TIMESTAMP(pay_time) >='{$start}' and UNIX_TIMESTAMP(pay_time)<='{$end}'");
  ...
  for ($j = 1; $j <= $pagesize; $j++) {
    $sql = "select * from " . tablename("pintuan_qgorder")." WHERE  store_id={$storeid}  and state in (2,3,4,5,7,8,9,10) and UNIX_TIMESTAMP(pay_time) >='{$start}' and UNIX_TIMESTAMP(pay_time)<='{$end}' ORDER BY id DESC limit " . ($j - 1) * 5000 . ",5000 ";
