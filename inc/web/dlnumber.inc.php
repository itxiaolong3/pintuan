...
$total=pdo_fetchcolumn("select count(*) from " . tablename("pintuan_number").$where." group by num ");
foreach($list as $key => $value){
	$num=$value['num'];
	$newsql=" select id,num,code from ".tablename('pintuan_number')." where uniacid={$_W['uniacid']} and store_id={$storeid}  and num='{$num}' and state=1  order by id asc";
...
if($_GPC['op']=="delete"){
	$result = pdo_delete('pintuan_number', array('id'=>$_GPC['id']));
		if($result){
