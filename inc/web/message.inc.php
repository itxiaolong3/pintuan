...
$sql=" select distinct user_id from".tablename('pintuan_formid')." where  uniacid={$_W['uniacid']} and time>={$time} ";
...
function set_msg($_W, $_GPC,$user){
 ...
 $userinfo=pdo_get('pintuan_user',array('id'=>$user));
 $time=time()-60*60*24*7;
 if($_POST['dz']){
 	$_POST['dz']=$_POST['dz'];
 }else{
 	$_POST['dz']="pintuan/pages/Liar/loginindex";
 }
  if($_POST['sj']){
 	$_POST['sj']=$_POST['sj'];
 }else{
 ...
 $form=pdo_fetch("select * from ".tablename('pintuan_formid')." where user_id={$user} and time>={$time} order by id asc");
$formwork ='{
...
pdo_insert('pintuan_message2',$data);
